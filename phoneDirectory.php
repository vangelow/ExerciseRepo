...
function phone($strng, $num) {
    // clean all
    $clean = preg_replace("/[^-0-9a-z\s+A-Z\n<>.']/", " ", $strng);
    // search num
    
    $a = explode("\n", $clean);
    
    $pattern = "/\\+$num/";
    $r = preg_grep($pattern, $a);


    if (count($r) > 1) return "Error => Too many people: ".$num;
    if (count($r) == 0) return "Error => Not found: ".$num;
    // replace num 
    $rr = preg_replace($pattern, "", $r[array_keys($r)[0]]);
    // name

    $pattern = "/<(.*)>/";
    preg_match($pattern, $rr, $m);
    $name = $m[1];
    // address
    $ad = trim(preg_replace($pattern, "", $rr));
    $ad = preg_replace("/[ ]{2,}/", " ", $ad);
    return "Phone => ".$num.", Name => ".$name.substr(1, strlen($name)-1).", Address => ".$ad;
}
